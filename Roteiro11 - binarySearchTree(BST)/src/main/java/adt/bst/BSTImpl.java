		...
		if (!node.isEmpty()) {
			if (node.isLeaf()) {
				...
				if (!isRoot(node)) {
					...
					BSTNode<T> saPoha = auxMaximum((BSTNode<T>) node.getLeft());
					saPoha.setRight(rightNode.getLeft());
					rightNode.getLeft().setParent(saPoha);
				}
			}
		}
	}
